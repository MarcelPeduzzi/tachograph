...
func TestEventsAndFaults_Gen1(t *testing.T) {
	// Discover all matching hexdump files
	hexdumpFiles, err := findHexdumpFiles(vuv1.TransferType_EVENTS_AND_FAULTS_GEN1)
	if err != nil {
		t.Fatalf("Failed to discover hexdump files: %v", err)
	}
	if len(hexdumpFiles) == 0 {
		t.Skip("No hexdump files found for EVENTS_AND_FAULTS_GEN1")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			marshaled, err := marshalOpts.MarshalEventsAndFaultsGen1(eventsAndFaults)
			if err != nil {
