...
func TestActivities_Gen2V1(t *testing.T) {
	// Discover all matching hexdump files
	hexdumpFiles, err := findHexdumpFiles(vuv1.TransferType_ACTIVITIES_GEN2_V1)
	if err != nil {
		t.Fatalf("Failed to discover hexdump files: %v", err)
	}
	if len(hexdumpFiles) == 0 {
		t.Skip("No hexdump files found for ACTIVITIES_GEN2_V1")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			marshaled, err := marshalOpts.MarshalActivitiesGen2V1(activities)
			if err != nil {
