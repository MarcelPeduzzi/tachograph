...
// Example: "testdata/records/000-anonymized/008-EF_PLACES-GENERATION_1-DATA.hexdump"
//
//	-> "testdata/records/000-anonymized/008-EF_PLACES-GENERATION_1-DATA.golden.json"
...
// findHexdumpFiles discovers all hexdump files matching the specified EF type, generation, and content type.
...
func findHexdumpFiles(
	efType cardv1.ElementaryFileType,
	generation ddv1.Generation,
	contentType cardv1.ContentType,
) ([]string, error) {
	// Convert enums to their string names
	efTypeName := efType.String()
	generationName := generation.String()
	contentTypeName := contentType.String()

	// Build the pattern we're looking for
	// Example: "*-EF_PLACES-GENERATION_1-DATA.hexdump"
	pattern := fmt.Sprintf("-%s-%s-%s.hexdump", efTypeName, generationName, contentTypeName)
