...
func TestPlaces_Generation1(t *testing.T) {
	...
	if len(hexdumpFiles) == 0 {
		t.Fatal("No hexdump files found for EF_PLACES GENERATION_1")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			places, err := opts.unmarshalPlaces(data)
			if err != nil {
			...
			marshaled, err := marshalOpts.MarshalPlaces(places)
			if err != nil {
