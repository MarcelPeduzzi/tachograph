...
func TestUnmarshalVehicleRegistration(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			input:         []byte{0x12, 0x0F, 0x46, 0x50, 0x41, 0x2D, 0x38, 0x32, 0x39, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_FINLAND,
			wantRegNumber: "FPA-829",
		},
		{
			name:          "Spain with registration ABC-1234",
			input:         []byte{0x0F, 0x0F, 0x41, 0x42, 0x43, 0x2D, 0x31, 0x32, 0x33, 0x34, 0x20, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_SPAIN,
			wantRegNumber: "ABC-1234",
		},
		{
			name:          "Germany with registration B-MW-1234",
			input:         []byte{0x0D, 0x00, 0x42, 0x2D, 0x4D, 0x57, 0x2D, 0x31, 0x32, 0x33, 0x34, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_GERMANY,
			wantRegNumber: "B-MW-1234",
		},
		{
			name:          "France with registration 75ABC12",
			input:         []byte{0x11, 0x00, 0x37, 0x35, 0x41, 0x42, 0x43, 0x31, 0x32, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_FRANCE,
			wantRegNumber: "75ABC12",
		},
		{
			name:          "Italy with registration AB123CD",
			input:         []byte{0x1A, 0x00, 0x41, 0x42, 0x31, 0x32, 0x33, 0x43, 0x44, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_ITALY,
			wantRegNumber: "AB123CD",
		},
		{
			name:          "Sweden with full 13-char registration",
			input:         []byte{0x2C, 0x00, 0x41, 0x42, 0x43, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30},
			wantNation:    ddv1.NationNumeric_SWEDEN,
			wantRegNumber: "ABC1234567890",
		},
		{
			name:          "Empty nation (0xFF) with empty registration",
			input:         []byte{0xFF, 0xFF, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
			wantNation:    ddv1.NationNumeric_NATION_NUMERIC_EMPTY,
			wantRegNumber: "",
		},
		{
			name:          "Default nation (0x00) with short registration",
			input:         []byte{0x00, 0x00, 0x41, 0x42, 0x43, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
	...
	tests := []struct {
	...
	}{
		{
			...
			vehicleReg: func() *ddv1.VehicleRegistrationIdentification {
				...
				num.SetLength(13)
				num.SetEncoding(ddv1.Encoding_ISO_8859_15)
				vr.SetNumber(num)
				return vr
			}(),
			want: []byte{0x12, 0x0F, 0x46, 0x50, 0x41, 0x2D, 0x38, 0x32, 0x39, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			name: "Spain with registration ABC-1234",
			vehicleReg: func() *ddv1.VehicleRegistrationIdentification {
			...
			want: []byte{0x0F, 0x0F, 0x41, 0x42, 0x43, 0x2D, 0x31, 0x32, 0x33, 0x34, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			...
			want: []byte{0x0D, 0x00, 0x41, 0x42, 0x43, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			...
			vehicleReg: func() *ddv1.VehicleRegistrationIdentification {
				...
				num.SetValue("")
				num.SetLength(13)
				num.SetEncoding(ddv1.Encoding_ENCODING_EMPTY)
				vr.SetNumber(num)
				return vr
			}(),
			want: []byte{0x1A, 0xFF, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			...
			want: []byte{0x11, 0x00, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			...
			want: []byte{0xFF, 0x00, 0x54, 0x45, 0x53, 0x54, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
...
func TestAppendVehicleRegistration_WithExistingData(t *testing.T) {
	...
	want := []byte{
		...
		0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, // padding
	}
...
func TestVehicleRegistrationRoundTrip(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "Italy with full 13-char registration",
			input: []byte{0x1A, 0x00, 0x41, 0x42, 0x43, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30},
		},
		{
			name:  "Sweden with short registration",
			input: []byte{0x2C, 0x00, 0x41, 0x42, 0x43, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			name:  "Empty nation with empty registration",
			input: []byte{0xFF, 0xFF, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
		{
			name:  "Default nation with registration",
			input: []byte{0x00, 0x00, 0x54, 0x45, 0x53, 0x54, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20},
		},
	}
