...
func TestVehicleUnitsUsed_Generation2(t *testing.T) {
	...
	if len(hexdumpFiles) == 0 {
		t.Fatal("No hexdump files found for EF_VEHICLE_UNITS_USED GENERATION_2")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			vehicleUnits, err := opts.unmarshalVehicleUnitsUsed(data)
			if err != nil {
			...
			loadOrCreateGolden(t, vehicleUnits, goldenPath)

			// Round-trip test
			marshalOpts := MarshalOptions{}
			marshaled, err := marshalOpts.MarshalCardVehicleUnitsUsed(vehicleUnits)
			if err != nil {
