...
func TestControlActivity_Generation1(t *testing.T) {
	...
	if len(hexdumpFiles) == 0 {
		t.Fatal("No hexdump files found for EF_CONTROL_ACTIVITY_DATA GENERATION_1")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			controlActivity, err := opts.unmarshalControlActivityData(data)
			if err != nil {
			...
			loadOrCreateGolden(t, controlActivity, goldenPath)

			// Round-trip test
			marshalOpts := MarshalOptions{}
			marshaled, err := marshalOpts.MarshalCardControlActivityData(controlActivity)
			if err != nil {
...
func TestControlActivity_Generation2(t *testing.T) {
	// Discover all matching hexdump files using type-safe enums
	hexdumpFiles, err := findHexdumpFiles(
		cardv1.ElementaryFileType_EF_CONTROL_ACTIVITY_DATA,
	...
	if len(hexdumpFiles) == 0 {
		t.Fatal("No hexdump files found for EF_CONTROL_ACTIVITY_DATA GENERATION_2")
	}
