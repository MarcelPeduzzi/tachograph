...
func TestDetailedSpeed_Gen1(t *testing.T) {
	// Discover all matching hexdump files
	hexdumpFiles, err := findHexdumpFiles(vuv1.TransferType_DETAILED_SPEED_GEN1)
	if err != nil {
		t.Fatalf("Failed to discover hexdump files: %v", err)
	}
	if len(hexdumpFiles) == 0 {
		t.Skip("No hexdump files found for DETAILED_SPEED_GEN1")
	}
	...
	for _, hexdumpPath := range hexdumpFiles {
		...
		t.Run(testName, func(t *testing.T) {
			...
			marshaled, err := marshalOpts.MarshalDetailedSpeedGen1(detailedSpeed)
			if err != nil {
